...
# Modded Min Conflicts
def my_min_conflicts(csp, max_steps=250):
...
def run_q4_one_graph(friend_group):
    ...
    for group_num in range(len(variable)):
        ...
        solution = my_min_conflicts(csp_friend)
...
def generate_graphs(n, graphs):
    for i in range(n):
        graph = [rand_graph(0.1, 105), rand_graph(0.2, 105), rand_graph(0.3, 105),
                rand_graph(0.4, 105), rand_graph(0.5, 105), rand_graph(0.6, 105)]
        graphs.append(graph)

def print_results(graph):
    elapsed_time, csp_assign, csp_unassign, number_of_teams, number_of_prunes, solution, group_dict = run_q4_one_graph(graph)
    print("===========================================================")
    print("Graph: ", graph)
...
def run_q4():
    ...
    for i in range(5):
        print(f'Trial {i + 1} Running Graph with p:0.1, n:105...')
        print_results(graphs[i][0])
        print(f'Trial {i + 1} Running Graph with p:0.2, n:105...')
        print_results(graphs[i][1])
        print(f'Trial {i + 1} Running Graph with p:0.3, n:105...')
        print_results(graphs[i][2])
        print(f'Trial {i + 1} Running Graph with p:0.4, n:105...')
        print_results(graphs[i][3])
        print(f'Trial {i + 1} Running Graph with p:0.5, n:105...')
        print_results(graphs[i][4])
        print(f'Trial {i + 1} Running Graph with p:0.6, n:105...')
